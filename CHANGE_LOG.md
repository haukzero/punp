...
- 2025.12.20
    - 支持更多的配置规则功能
- 2025.12.18
    - 支持使用 `**` 进行目录多跳匹配, 对于隐藏目录可通过添加 `-H` 选项展开多跳
    - 支持版本更新选项(stable/nightly), 默认为 stable. 对于 stable, 将只更新到上一次标记 tag 的位置; 对于 nightly, 将更新到最新的一次 commit
    - 对于 latex 文件, 支持通过启用选项 `--enable-latex-jumping` 尝试递归的查找并处理 `\input` 或 `\includ` 的 latex 文件
    - 重构构建保护区域与分页逻辑, 让保护区域单独为一页, 并将构建保护区域索引功能移入 `FileProcessor` 中, 简化 AC 自动机逻辑, 使各组件分工更明确
- 2025.12.17
    - 添加选项 `-n`, 支持试运行而不实际处理文件, 仅打印将要处理文件的路径
    - 更新了颜色打印处理逻辑, 让绿色, 蓝色, 红色更明亮, 同时处理在打印结果重定向到文件时对颜色转义符的处理逻辑
    - BRAKING_CHANGE: 重构了项目结构, 使其更模块化
